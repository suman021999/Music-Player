...
const Home = ({ setCurrentTrack, searchQuery }) => {
  const [musicList, setMusicList] = useState([]);
  ...
  useEffect(() => {
    if (!searchQuery) {
      ...
      const filtered = musicList.filter(music => {
        const musicName = music.text || music.originalname || "";
        return musicName.toLowerCase().includes(searchQuery.toLowerCase());
      });
